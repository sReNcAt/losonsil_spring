...
<script>
	...
	//검색 초기화
	var setRequestInit = function (paramRequest, isChangeClass = true) {
	    var request;
	    if (paramRequest == null) {
	        request = new Object();
	        request.firstCategory = 0;
	        request.secondCategory = 0;
	        var classCookie = getCookie('MarketClassSearch');
	        request.classNo = classCookie;
	        $("input[type='radio'][name='deal-classNo']:radio[value='" + request.classNo + "']").prop("checked", true).click();
	    }
	    else {
	        request = paramRequest;
	        if (isChangeClass) { //isChangeClass가 True 경우만 클래스 변경
	            var classCookie = getCookie('MarketClassSearch');
	            if (classCookie != null || classCookie != "") {
	                request.classNo = classCookie;
	            }
	            else {
	                request.classNo = '';
	...
	var searchDetailModalInit = function (request) {
	    ...
	    for (var i = 0; i < 3; i++) {
	        if (request.skillOptionList != null && request.skillOptionList[i].firstOption != "" && request.skillOptionList[i].firstOption != undefined) {// 첫번째옵션
	    ...
	    //옵션 1개 이상이면 토글 Active
	    if (skillOptionCount > 1) {
	        if ($("#btnSkillOptionToggle").hasClass("is-active") === true) {
	        }
	        else {
	            $("#btnSkillOptionToggle").trigger('click');
	        }
	    }
	    else {
	        if ($("#btnSkillOptionToggle").hasClass("is-active") === true) {
	    ...
	    for (var i = 0; i < 3; i++) {
	        if (request.etcOptionList != null && request.etcOptionList[i].firstOption != "" && request.etcOptionList[i].firstOption != undefined) { //첫번째 옵션
	            var name = 'deal-etc' + i;
	    ...
	    //옵션 1개 이상이면 토글 Active
	    if (etcOptionCount > 1) {
	        if ($("#btnEtcOptionToggle").hasClass("is-active") === true) {
	        }
	        else {
	            $("#btnEtcOptionToggle").trigger('click');
	        }
	    }
	    else {
	        if ($("#btnEtcOptionToggle").hasClass("is-active") === true) {
	    ...
	    if (etcId == "") {
	        ...
	        if (etcId == 3 && classNo != undefined && classNo != '') { //각인효과는 클래스 선택 여부에 따라 직업각인 노출
	            ...
	            if (etcSubList.length > 1) {
	                etcSubList.sort(function (a, b) { //클래스가 선택되어 있다면 직업각인 먼저 노출
	...
	//직업 선택 시 쿠키저장
	$("input:radio[name=deal-classNo]").click(function () {
	    if ($(this).is(':checked')) {
	        setCookie('MarketClassSearch', $(this).val()); //클래스 쿠키저장
	    }
	})
	...
	//자동완성 된 아이템 클릭
	$(".deal-contents").on('click', '.name .words a', e => {
	...
	$(".deal-contents").on('keydown', '.name', e => {
	    ...
	    if (e.keyCode === 40) {
	        ...
	        if (active.is(words.find('a')) && !active.is(words.find('a').last())) {
	            const next = active.parent().next().find('a');
	            next.focus();
	        }
	        e.preventDefault();
	    }
	    if (e.keyCode === 38) {
	        ...
	        if (active.is(words.find('a')) && !active.is(words.find('a').first())) {
	            const prev = active.parent().prev().find('a');
	            prev.focus();
	        }
	        e.preventDefault();
	    }
