package srencat.utils.security;
...
public class JwtManager {
    ...
    public static String generateToken(String idx, String name, String employeeNumber, String phone) {
        ...
        String tokenStr = Jwts.builder()
                        ...
                        .setIssuer("JNPSOLITION").setId(id)
                        .setIssuedAt(now)
                        .setNotBefore(now)
                        .setExpiration(exp)
                        .claim("idx", idx)
                        ...
                        .signWith(SignatureAlgorithm.HS256, "jnpsolutionJWT".getBytes()) // 개발시에만사용할것
                        .compact();
        return tokenStr;
    }
    	...
    	if ( token.equals(null) || token.equals("") ) {
            ...
            try {
                Claims claims = Jwts.parser()
                            ...
                            .setSigningKey("jnpsolutionJWT".getBytes()) // 개발시에만 사용할것
                            .parseClaimsJws(token).getBody();
                tokenMap.put("idx", claims.get("idx").toString());
                tokenMap.put("name", claims.get("name").toString());
                tokenMap.put("employeeNumber", claims.get("employeeNumber").toString());
                tokenMap.put("phone", claims.get("phone").toString());
